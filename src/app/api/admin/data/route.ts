...
export async function GET(req: NextRequest) {
  try {
    ...
    let filteredStudents = students.filter((student) => {
      // 1. Search filter
      if (searchQuery) {
        const matchesName = (student.fullName || '').toLowerCase().includes(searchQuery);
        const matchesEmail = (student.email || '').toLowerCase().includes(searchQuery);
        const matchesPhone = (student.whatsappNumber || '').includes(searchQuery);
        const matchesAdmission = (student.admissionNumber || '').toLowerCase().includes(searchQuery);
